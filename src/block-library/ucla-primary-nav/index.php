...
if (defined('IS_GUTENBERG_PLUGIN') && IS_GUTENBERG_PLUGIN) {
	 ...
	 */
	function block_ucla_primary_nav_get_menu_items_at_location($location)
	{
		if (empty($location)) {
			return;
		}
	 ...
	 */
	function block_ucla_primary_nav_sort_menu_items_by_parent_id($menu_items)
	{
	 ...
	 */
	function block_ucla_primary_nav_parse_blocks_from_menu_items($menu_items, $menu_items_by_parent_id)
	{
		if (empty($menu_items)) {
			return array();
		}
		...
		foreach ($menu_items as $menu_item) {
			...
			$block = array(
				'blockName' => isset($menu_items_by_parent_id[$menu_item->ID]) ? 'ucla-wcl-wpblocks/primary-nav-submenu' : 'ucla-wcl-wpblocks/primary-nav-link',
				'attrs'     => array(
			...
			$block['innerBlocks']  = isset($menu_items_by_parent_id[$menu_item->ID])
				? block_ucla_primary_nav_parse_blocks_from_menu_items($menu_items_by_parent_id[$menu_item->ID], $menu_items_by_parent_id)
				: array();
 ...
 */
function block_ucla_primary_nav_build_css_colors($attributes)
{
	$colors = array(
		'css_classes'           => array(),
		'inline_styles'         => '',
 ...
 */
function block_ucla_primary_nav_build_css_font_sizes($attributes)
{
	// CSS classes.
	$font_sizes = array(
		'css_classes'   => array(),
 ...
 * @return string
 */
function block_ucla_primary_nav_render_submenu_icon()
{
 ...
 */
function block_ucla_primary_nav_get_classic_menu_fallback()
{
 ...
 */
function block_ucla_primary_nav_get_classic_menu_fallback_blocks($classic_nav_menu)
{
	...
	$inner_blocks = block_ucla_primary_nav_parse_blocks_from_menu_items(
 ...
 * @return array the normalized parsed blocks.
 */
function block_ucla_primary_nav_maybe_use_classic_menu_fallback()
{
	// See if we have a classic menu.
	$classic_nav_menu = block_ucla_primary_nav_get_classic_menu_fallback();
	...
	$classic_nav_menu_blocks = block_ucla_primary_nav_get_classic_menu_fallback_blocks($classic_nav_menu);
	...
	return block_ucla_primary_nav_get_most_recently_published_navigation();
}
 ...
 */
function block_ucla_primary_nav_get_most_recently_published_navigation()
{
 ...
 * @return array the normalized parsed blocks.
 */
function block_ucla_primary_nav_filter_out_empty_blocks($parsed_blocks)
{
 ...
 */
function block_ucla_primary_nav_get_fallback_blocks()
{
	...
	$navigation_post = block_ucla_primary_nav_get_most_recently_published_navigation();

	// If there are no navigation posts then try to find a classic menu
	// and convert it into a block based navigation menu.
	if (!$navigation_post) {
		$navigation_post = block_ucla_primary_nav_maybe_use_classic_menu_fallback();
	}

	// Use the first non-empty Navigation as fallback if available.
	if ($navigation_post) {
		$maybe_fallback = block_ucla_primary_nav_filter_out_empty_blocks(parse_blocks($navigation_post->post_content));
	 ...
	 */
	return apply_filters('block_ucla_primary_nav_render_fallback', $fallback_blocks);
}
 ...
 */
function block_ucla_primary_nav_get_post_ids($inner_blocks)
{
	$post_ids = array_map('block_ucla_primary_nav_from_block_get_post_ids', iterator_to_array($inner_blocks));
...
function block_ucla_primary_nav_from_block_get_post_ids($block)
{
	...
	if ($block->inner_blocks) {
		$post_ids = block_ucla_primary_nav_get_post_ids($block->inner_blocks);
	}

	if ('ucla-wcl-wpblocks/primary-nav-link' === $block->name || 'ucla-wcl-wpblocks/primary-nav-submenu' === $block->name) {
...
function render_block_ucla_primary_nav($attributes, $content, $block)
{
	...
	if (
		...
		!empty(block_ucla_primary_nav_get_menu_items_at_location($attributes['__unstableLocation']))
	) {
		$menu_items = block_ucla_primary_nav_get_menu_items_at_location($attributes['__unstableLocation']);
		if (empty($menu_items)) {
			return '';
		}

		$menu_items_by_parent_id = block_ucla_primary_nav_sort_menu_items_by_parent_id($menu_items);
		$parsed_blocks           = block_ucla_primary_nav_parse_blocks_from_menu_items($menu_items_by_parent_id[0], $menu_items_by_parent_id);
	...
	if (array_key_exists('ref', $attributes)) {
		...
		if ('publish' === $navigation_post->post_status) {
			...
			$compacted_blocks = block_ucla_primary_nav_filter_out_empty_blocks($parsed_blocks);
	...
	if (empty($inner_blocks)) {
		...
		$fallback_blocks = block_ucla_primary_nav_get_fallback_blocks();
	 ...
	 */
	$inner_blocks = apply_filters('block_ucla_primary_nav_render_inner_blocks', $inner_blocks);
 ...
 *
 * @uses render_block_ucla_primary_nav()
 * @throws WP_Error An WP_Error exception parsing the block definition.
 */
function register_block_ucla_primary_nav()
{
	global $ucla_wcl_plugin_blocks_build_dir;
	register_block_type_from_metadata(
		$ucla_wcl_plugin_blocks_build_dir . 'ucla-primary-nav',
		array(
			'render_callback' => 'render_block_ucla_primary_nav',
		)
	);
}

add_action('init', 'register_block_ucla_primary_nav');
...
add_filter('render_block_data', 'block_ucla_primary_nav_typographic_presets_backcompatibility');
