...
export default function NavigationSubmenuEdit({
		...
		(select) => {
			...
			if (selectedBlockChildren?.length === 1) {
				...
				_onlyDescendantIsEmptyLink =
					singleBlock?.name === 'ucla-wcl-wpblocks/ucla-primary-nav-link' &&
	...
	const allowedBlocks = isAtMaxNesting
		? ALLOWED_BLOCKS.filter(
			(blockName) => blockName !== 'ucla-wcl-wpblocks/ucla-primary-nav-submenu'
		)
		: ALLOWED_BLOCKS;
	...
	function transformToLink() {
		const newLinkBlock = createBlock('ucla-wcl-wpblocks/ucla-primary-nav-link', attributes);
